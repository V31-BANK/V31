...
public class TransferLimitController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<TransferLimitResponse>> create(@RequestBody TransferLimitRequest request) {
        ApiResponse<TransferLimit> result = this.transferLimitInputPort.create(request.code(), request.name(), request.dailyMax());
        if (!result.success()) {
    ...
    public ApiResponse<PageResponse<TransferLimitResponse>> page(TransferLimitPageQuery query) {
        PageResult<TransferLimitResponse> page = this.transferLimitInputPort.page(query).map(TransferLimitResponse::from);
        return ApiResponse
            .ok(PageResponse.of(page.getRecords(), page.getTotal(), page.getPageNumber(), page.getPageSize()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<TransferLimitResponse>> update(@PathVariable UUID id, @RequestBody TransferLimitRequest request) {
        return toResponseEntity(this.transferLimitInputPort.update(id, request.code(), request.name(), request.dailyMax(),
                request.status()));
    }
    ...
    private static ResponseEntity<ApiResponse<TransferLimitResponse>> toResponseEntity(ApiResponse<TransferLimit> result) {
