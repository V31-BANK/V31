 ...
 * <p>
 * This is written by hand rather than generated. Code generation would need a
 * live database at build time, which would make the build fail wherever one is
 * not reachable — a fresh checkout, a build agent — for a service with a single
 * table. What is written here is the same shape the generator emits, and moving
 * to generation later replaces this file without touching anything that uses it.
 * <p>
 * It extends {@link TableImpl} and declares a primary key on purpose: that is
 * what makes {@link ComplianceCaseRecord} an updatable record, and updatable
 * records are what the audit listener registered by the jOOQ starter listens to.
 * A table without a key would still read and write, but nothing would fill in who
 * wrote the row and when.
 * <p>
 * The fields are named in upper case, against the surrounding convention, because
 * that is how a generated table reads and how every jOOQ query is written:
 * {@code COMPLIANCE_CASE.CASE_NUMBER.eq(...)}.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public class ComplianceCaseTable extends TableImpl<ComplianceCaseRecord> {
    ...
    /**
     * {@code compliance_case.id}, a time-ordered UUIDv7 issued by the audit
     * listener when the caller does not supply one.
     */
    ...
    /**
     * Declared after the fields it names, since instance initialisers run in the
