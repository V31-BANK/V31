 ...
 * <p>
 * Valkey has no tables, so the layout is the schema. Written down here it can be
 * read; scattered across the adapter as string concatenation it could only be
 * reconstructed:
 *
 * <pre>
 ...
 * </pre>
 *
 * The three sorted sets are what make a page cheap. A key-value store cannot
 * offer "the tenth page, newest first" over a keyspace; it can offer a range of a
 * sorted set in logarithmic time, so the ordering is maintained as products are
 * written rather than computed as they are read.
 * <p>
 * Segments go through {@link ValkeyKeys}, which refuses any containing a colon —
 * without that, a product code carrying one would address a key outside this
 * namespace.
 *
 * @author Xander Wang
 * @since 0.2.0
