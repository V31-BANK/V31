 ...
 * <p>
 * The counterpart of a persistence adapter: everything that knows the accounts
 * come from another service over a wire is confined here, and the application
 * layer above sees a port like any other.
 *
 * <h2>Failures are translated, not propagated</h2>
 *
 * Every call goes through {@link GrpcErrors}, so a refusal from the ledger
 * arrives here as a {@link BusinessException} carrying the code the ledger
 * reported rather than as a {@link StatusRuntimeException}. Without that, a
 * status code would leak into the application layer and every caller would have
 * to know what {@code ALREADY_EXISTS} means.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
...
public class LedgerAccountGrpcAdapter implements LedgerAccountPort {
     ...
     * <p>
     * Every other refusal is raised: an account this service is not allowed to
     * see, or a ledger that is down, is not the same as an account that does not
     * exist, and returning empty for those would hide a problem behind a blank
     * result.
     */
    @Override
    public Optional<LedgerAccountSummary> findById(UUID id) {
        try {
            return Optional.of(toSummary(this.ledger
                .getLedgerAccount(GetLedgerAccountRequest.newBuilder().setId(id.toString()).build())));
        }
    ...
    public PageResponse<LedgerAccountSummary> findPage(int pageNumber, int pageSize, String code) {
        ...
        ListLedgerAccountsResponse response = GrpcErrors
            .call(() -> this.ledger.listLedgerAccounts(request.build()));
    ...
    public LedgerAccountSummary delete(UUID id) {
        return toSummary(GrpcErrors.call(() -> this.ledger
     ...
     * template should read {@code ASSET}, not
     * {@code LEDGER_ACCOUNT_TYPE_ASSET}.
     ...
     * "unspecified", which the ledger would read as "no filter" or "no type" and
     * act on silently.
    ...
    private static LedgerAccountType toType(String type) {
        ...
        return (value != LedgerAccountType.UNRECOGNIZED) ? value
                : LedgerAccountType.LEDGER_ACCOUNT_TYPE_UNSPECIFIED;
    }
