 ...
 * <p>
 * Neither knows about the other, which is the point: the proto file can gain a
 * field without the domain changing, and the domain can be reshaped without
 * breaking a caller compiled against an older copy of the contract. Everything
 * that would otherwise couple them is spent here.
 *
 * <h2>Enums do not map by name</h2>
 *
 * Proto3 requires every enum to have a zero value, and gives it to whatever is
 * listed first unless one is reserved for "not set". This contract reserves one,
 * so the names differ from the domain's by a prefix and by that extra member.
 * Translating by {@code valueOf} would work until the day someone reorders the
 * proto, so the mapping is written out.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public final class LedgerAccountProtos {
     ...
     * @return the type, or {@code null} when the caller left it unset — which the
     * caller is entitled to do, and which each request decides the meaning of
     */
     ...
     * <p>
     * The field is a string on the wire because protobuf has no UUID, so this is
     * the boundary where that string has to become an identifier or be rejected.
    ...
    /**
     * Return the code a failed outcome reported, so that it can be raised as an
     * exception the transport knows how to render.
