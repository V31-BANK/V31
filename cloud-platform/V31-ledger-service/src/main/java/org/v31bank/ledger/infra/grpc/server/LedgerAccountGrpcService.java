 ...
 * implementation of them. Nothing is decided here: the request is translated, the
 * use case is asked, and the outcome is translated back.
 *
 * <h2>How a refusal becomes a status</h2>
 *
 * The use cases report an outcome rather than throwing, because that is what
 * suits an HTTP response. gRPC has no envelope: a call either returns its message
 * or fails with a status. So a refused outcome is raised as a
 * {@link BusinessException} here, and the handler the gRPC starter registers
 * turns it into a status carrying the same code the REST layer would have put in
 * the envelope. A caller therefore sees the same {@code CONFLICT} either way.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
...
public class LedgerAccountGrpcService extends LedgerAccountServiceGrpc.LedgerAccountServiceImplBase {
    ...
    public void createLedgerAccount(CreateLedgerAccountRequest request,
            StreamObserver<org.v31bank.ledger.api.v1.LedgerAccount> observer) {
    ...
    public void getLedgerAccount(GetLedgerAccountRequest request,
        ...
        LedgerAccount account = this.ledgerAccountInputPort.get(id)
            .orElseThrow(() -> new BusinessException(CommonErrorCode.NOT_FOUND,
                    "No ledger account exists with id " + id));
        respond(observer, account);
    }
    ...
    public void updateLedgerAccount(UpdateLedgerAccountRequest request,
            StreamObserver<org.v31bank.ledger.api.v1.LedgerAccount> observer) {
        LedgerAccount account = unwrap(this.ledgerAccountInputPort.update(
                LedgerAccountProtos.toUuid(request.getId()), request.getCode(), request.getName(),
                LedgerAccountProtos.fromProto(request.getType()),
    ...
    public void deleteLedgerAccount(DeleteLedgerAccountRequest request,
            StreamObserver<org.v31bank.ledger.api.v1.LedgerAccount> observer) {
        respond(observer, unwrap(this.ledgerAccountInputPort.delete(LedgerAccountProtos.toUuid(request.getId()))));
    }
    ...
    private static LedgerAccount unwrap(ApiResponse<LedgerAccount> result) {
        if (!result.success()) {
            throw new BusinessException(LedgerAccountProtos.errorCodeOf(result.code()), result.message());
        }
        return result.data();
    }

    private static void respond(StreamObserver<org.v31bank.ledger.api.v1.LedgerAccount> observer,
            LedgerAccount account) {
        observer.onNext(LedgerAccountProtos.toProto(account));
        observer.onCompleted();
    }
