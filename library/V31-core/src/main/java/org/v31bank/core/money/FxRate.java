 ...
 * <p>
 * The rate is directional: it says how much {@link #quote()} one unit of
 * {@link #base()} buys, and {@link #convert(Money)} refuses anything not
 * denominated in the base. Getting that backwards silently is a class of bug that
 * only shows up in the reconciliation, which is why it fails loudly instead.
 * <p>
 * {@link #asOf()} is carried because a crypto rate is worth minutes at most.
 * Pricing a movement against a rate that has gone stale is a real loss, so the
 * age is checked before the rate is used rather than assumed.
 *
 * @param base the asset being priced
 * @param quote the asset it is priced in
 * @param rate how much of the quote asset one unit of the base buys, always
 * greater than zero
...
public record FxRate(Asset base, Asset quote, BigDecimal rate, Instant asOf) {
    ...
    /**
     * The precision inversion works at, chosen to be far finer than any asset's
     * own so that the inverted rate is limited by the input rather than by this
     * calculation.
     */
    ...
    public FxRate {
        ...
        if (rate.signum() <= 0) {
            throw new IllegalArgumentException("A %s/%s rate must be greater than zero, not %s"
                .formatted(base.code(), quote.code(), rate.toPlainString()));
        }
    }

    /**
     * Convert an amount of the base asset into the quote asset, rounding to what
     * the quote asset can represent using banker's rounding, which does not drift
     * in the bank's favour or the customer's over many conversions.
    ...
    /**
     * Convert an amount of the base asset into the quote asset, rounding as
     * asked — a quote given to a customer is usually rounded against the party
     * that requested it rather than neutrally.
     * @param amount the amount to convert, denominated in the base asset
     ...
     * <p>
     * Inversion divides, so the result is only as good as the precision it is
     * computed at and will not always round-trip back to this rate exactly. Where
     * both directions are quoted — which is normally the case, since a market
     * spreads them apart rather than mirroring them — take the quoted one instead.
    ...
    /**
     * Whether this rate was observed longer ago than the caller is willing to
     * accept.
