 ...
 * <p>
 * Logs outlive the request that wrote them and travel further: they are shipped
 * to an aggregator, replicated, retained for years, and read by people who have
 * no business seeing an account number. Anything identifying goes through here
 * first, leaving only enough for a human to confirm they are looking at the right
 * record.
 * <p>
 * The mask is a fixed length rather than one character per hidden character, so
 * the masked form does not disclose how long the original was.
 * <p>
 * A value too short to mask safely is hidden completely: revealing the last four
 * digits of a six digit value gives away most of it.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public final class Masks {
    ...
    /**
     * Mask an email address, keeping the first character and the domain so that a
     * support conversation can confirm the address without the log holding it.
    ...
    /**
     * Shorten a blockchain address the way a wallet does, keeping both ends so it
     * can be recognised at a glance.
     * <p>
     * An address is public, so this is for readability rather than secrecy — a
     * full address is forty odd characters of noise that makes a log line
     * unreadable. It is not a substitute for the care an address deserves
     * elsewhere: the middle is exactly where an address-swapping attack hides, so
     * an address a customer is asked to verify must be shown in full.
    ...
    /**
     * Hide a value completely, for the ones with no readable part at all — an API
     * key, a token, a private key, a password.
