 ...
 * <p>
 * A version 7 identifier leads with a millisecond timestamp, so identifiers generated one
 * after another sort one after another. That matters for anything indexed: a random
 * version 4 identifier lands in a different page of the index on every insert, and a
 * table taking a high write rate spends its time splitting pages that a sequential key
 * would have appended to.
 * <p>
 * Entities persisted through Hibernate get theirs from
 * {@code @UuidGenerator(style = VERSION_7)} on {@code BaseEntity}. This class is for the
 * identifiers nothing else issues: domain events, outbox records, correlation
 * identifiers, and anything created before it reaches a repository.
 * <p>
 * Identifiers issued within the same millisecond are strictly increasing, and the
 * generator does not go backwards if the system clock does. The random parts come from
 * {@link SecureRandom}, so an identifier is not guessable from the ones around it — an
 * identifier that appears in a URL should not tell its holder what the next one will be.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public final class Uuids {
	...
	/**
	 * Each millisecond starts its counter in the bottom quarter of that range, so the
	 * value is unpredictable while still leaving room for at least 3072 identifiers
	 * before the millisecond has to be carried forward.
	 */
	 ...
	 * @throws IllegalArgumentException if the identifier is not a UUIDv7, since no other
	 * version carries a timestamp in this position
	 */
	 ...
	 * <p>
	 * Within one millisecond the counter carries the ordering. When it runs out, or when
	 * the clock steps backwards — an NTP correction, a leap second adjustment — the
	 * generator borrows from the next millisecond instead of reusing a timestamp it has
	 * already issued.
