 ...
 * <p>
 * Every amount is kept at exactly the asset's {@link Asset#scale() scale}, so a
 * balance can only ever be a whole number of satoshis, wei or cents. Amounts that
 * do not land on that granularity are refused rather than quietly rounded:
 * {@code Money.of(Asset.USD, new BigDecimal("1.005"))} throws, because rounding
 * money without being asked to is how a system loses a fraction of every
 * transaction. Where rounding is the intent, it is requested explicitly and the
 * {@link RoundingMode} is part of the call.
 * <p>
 * Amounts are signed. A ledger needs both directions — a debit and its matching
 * credit, a balance overdrawn against a credit line — so the rule that a customer
 * balance cannot go below zero belongs to the domain that owns that balance, not
 * to this type.
 * <p>
 * There is no {@code double} anywhere in this class, and none should be let near
 * one: 0.1 + 0.2 is not 0.3 in binary floating point, and a bank that computes
 * balances that way will not reconcile.
 * <p>
...
public final class Money implements Comparable<Money> {
    ...
    /**
     * Always scaled to {@code asset.scale()}, which is what makes the unscaled
     * value the amount in minor units and lets equality compare exactly.
     */
     ...
     * @return the amount
     * @throws IllegalArgumentException if the value carries more precision than
     * the asset allows
     */
    ...
    /**
     * Create an amount from its decimal text, the form amounts arrive in over the
     * wire and leave in towards a database.
     * @param asset the asset the amount is denominated in
     * @param amount the amount, for example {@code "0.00012345"}
     * @return the amount
     * @throws IllegalArgumentException if the text is not a number, or carries
     * more precision than the asset allows
     */
     ...
     * <p>
     * Reserved for the places where rounding is the decision being made — pricing
     * a quote, charging a fee — and the direction has been chosen on purpose.
     * Rounding a customer's balance is not one of them.
     * @param asset the asset the amount is denominated in
    ...
    /**
     * Create an amount from a count of the asset's smallest units, the form
     * blockchains and card networks use.
     * @param asset the asset the amount is denominated in
     * @param minorUnits the number of satoshis, cents, or whatever the asset
     * divides into
     * @return the amount
     */
    ...
    /**
     * Scale this amount by a factor — a fee rate, an interest rate — rounding the
     * result to what the asset can represent.
     ...
     * <p>
     * Splitting an amount between recipients is {@link #allocate(int)} instead:
     * dividing and rounding each share loses or invents the remainder, which
     * {@code allocate} distributes rather than discarding.
    ...
    /**
     * Return this amount with the opposite sign, which is how one side of a
     * double entry is derived from the other.
    ...
    /**
     * Split this amount into equal shares, giving the remainder away one minor
     * unit at a time so that the shares add back up to the original exactly.
     * <p>
     * Ten dollars split three ways is 3.34, 3.33 and 3.33 — never three times
     * 3.3333 rounded, which reconciles to a cent short.
    ...
    /**
     * Split this amount in the given proportions, giving the remainder to the
     * earlier shares one minor unit at a time so that the shares add back up to
     * the original exactly.
     * @param weights the proportions, which must not be negative and must not all
     * be zero
    ...
    /**
     * Return this amount as a count of the asset's smallest units — satoshis for
     * bitcoin, wei for ether — which is the form a blockchain expects.
    ...
    /**
     * Whether this amount exceeds another of the same asset, for the limit and
     * balance checks that read better than a comparison against zero.
    ...
    /**
     * Return the amount and its asset, never in scientific notation, so that a log
     * line reads {@code 0.00012345 BTC} rather than {@code 1.2345E-4 BTC}.
