...
/**
 * Integration tests for the RiskRule endpoints.
 * <p>
...
class RiskRuleApiIntegrationTests {

	private static final String PATH = "/api/v1/risk-rules";
	...
	@Autowired
	private JpaRiskRuleRepository records;
	...
	void createsARecordAndSaysWhereItWent() {
		...
		assertThat(data(body)).containsEntry("code", "RULE-0001")
	...
	void issuesATimeOrderedIdentifierAndStampsTheAuditFields() {
		Map<String, Object> created = create("RULE-0001", "First");
	...
	void findsARecordItJustCreated() {
		String id = (String) create("RULE-0001", "First").get("id");
		assertThat(data(get(PATH + "/" + id))).containsEntry("code", "RULE-0001");
	}

	@Test
	void refusesADuplicate() {
		create("RULE-0001", "First");
		Map<String, Object> body = this.client.post()
			.uri(PATH)
			.body(request("RULE-0001", "Duplicate"))
			.exchange()
			.expectStatus()
	...
	void updatesARecord() {
		String id = (String) create("RULE-0001", "First").get("id");
		this.client.put().uri(PATH + "/" + id).body(update("RULE-0001", "Renamed")).exchange().expectStatus().isOk();
	...
	private static Map<String, Object> request(String code, String name) {
		return Map.of("code", code, "name", name, "severity", "HIGH");
	}

	private static Map<String, Object> update(String code, String name) {
		return Map.of("code", code, "name", name, "severity", "HIGH", "status", "ACTIVE");
	}
	...
	private void createMany(int count) {
		for (int i = 1; i <= count; i++) {
			create("RULE-%04d".formatted(i), "Record " + i);
		}
	}
	...
	void rejectsAValueLongerThanTheColumnHolds() {
		Map<String, Object> body = this.client.post()
			.uri(PATH)
			.body(Map.of("code", "RULE-1", "name", TOO_LONG, "severity", "HIGH"))
			.exchange()
			.expectStatus()
			.isBadRequest()
			.expectBody(JSON_OBJECT)
