...
public class WalletAddressController {
	...
	@PostMapping
	public ResponseEntity<ApiResponse<WalletAddressResponse>> create(@Valid @RequestBody WalletAddressRequest request) {
		ApiResponse<WalletAddress> result = this.walletAddressInputPort.create(request.address(), request.label(),
				request.asset());
		if (!result.success()) {
	...
	public ApiResponse<PageResponse<WalletAddressResponse>> page(WalletAddressPageQuery query) {
		PageResult<WalletAddressResponse> page = this.walletAddressInputPort.page(query)
	...
	@PutMapping("/{id}")
	public ResponseEntity<ApiResponse<WalletAddressResponse>> update(@PathVariable UUID id,
			@Valid @RequestBody WalletAddressRequest request) {
		return toResponseEntity(this.walletAddressInputPort.update(id, request.address(), request.label(),
				request.asset(), request.status()));
	}
	...
	private static ResponseEntity<ApiResponse<WalletAddressResponse>> toResponseEntity(
			ApiResponse<WalletAddress> result) {
