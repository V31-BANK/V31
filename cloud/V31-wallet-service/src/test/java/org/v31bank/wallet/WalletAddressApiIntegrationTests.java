...
/**
 * Integration tests for the WalletAddress endpoints.
 * <p>
...
class WalletAddressApiIntegrationTests {

	private static final String PATH = "/api/v1/wallet-addresses";
	...
	@Autowired
	private JpaWalletAddressRepository records;
	...
	void createsARecordAndSaysWhereItWent() {
		Map<String, Object> body = this.client.post()
			.uri(PATH)
			.body(request("bc1qtest000000000000000000000000000001", "First"))
			.exchange()
			.expectStatus()
			.isCreated()
		...
		assertThat(data(body)).containsEntry("address", "bc1qtest000000000000000000000000000001")
			.containsEntry("label", "First")
	...
	void issuesATimeOrderedIdentifierAndStampsTheAuditFields() {
		Map<String, Object> created = create("bc1qtest000000000000000000000000000001", "First");
	...
	void findsARecordItJustCreated() {
		String id = (String) create("bc1qtest000000000000000000000000000001", "First").get("id");
		assertThat(data(get(PATH + "/" + id))).containsEntry("address", "bc1qtest000000000000000000000000000001");
	}

	@Test
	void refusesADuplicate() {
		create("bc1qtest000000000000000000000000000001", "First");
		Map<String, Object> body = this.client.post()
			.uri(PATH)
			.body(request("bc1qtest000000000000000000000000000001", "Duplicate"))
			.exchange()
			.expectStatus()
	...
	void updatesARecord() {
		...
		this.client.put()
			.uri(PATH + "/" + id)
			.body(update("bc1qtest000000000000000000000000000001", "Renamed"))
			.exchange()
			.expectStatus()
			.isOk();
		assertThat(data(get(PATH + "/" + id))).containsEntry("label", "Renamed").containsEntry("status", "ACTIVE");
	}
	...
	private static Map<String, Object> request(String address, String label) {
		return Map.of("address", address, "label", label, "asset", "BTC");
	}

	private static Map<String, Object> update(String address, String label) {
		return Map.of("address", address, "label", label, "asset", "BTC", "status", "ACTIVE");
	}

	private Map<String, Object> create(String address, String label) {
		return data(this.client.post()
			.uri(PATH)
			.body(request(address, label))
			.exchange()
			.expectStatus()
			.isCreated()
			.expectBody(JSON_OBJECT)
	...
	private void createMany(int count) {
		for (int i = 1; i <= count; i++) {
			create("bc1qtest" + "%030d".formatted(i), "Record " + i);
		}
	}
	...
	void rejectsAValueLongerThanTheColumnHolds() {
		Map<String, Object> body = this.client.post()
			.uri(PATH)
			.body(Map.of("address", "bc1qtest000000000000000000000000000001", "label", TOO_LONG, "asset", "BTC"))
			.exchange()
			.expectStatus()
			.isBadRequest()
			.expectBody(JSON_OBJECT)
			.returnResult()
			.getResponseBody();
		assertThat(violations(body)).extracting((violation) -> violation.get("field")).contains("label");
	}
