 ...
 * <p>
 * jOOQ has no equivalent of Spring Data's repository pagination, so the count query and
 * the windowed query are written out here rather than in every service that needs them —
 * and written out once, since the two are easy to let drift into disagreeing about which
 * rows they describe.
 * <p>
 * The query passed in must carry its own filtering and ordering but no limit: both the
 * count and the page are derived from it, so a query that already limits itself would
 * report a total it does not have. Ordering matters more than it looks — without it a
 * database is free to return rows in any order, and a row can appear on two consecutive
 * pages or on neither.
 *
 * @author Xander Wang
 * @since 0.2.0
...
public final class JooqPages {
	...
	/**
	 * The largest page a caller may ask for. A caller asking for more is given this
	 * rather than an error, because the alternative — honouring it — lets one request
	 ...
	 * <p>
	 * The count runs first and the page is skipped entirely when nothing matched, which
	 * is the common case for a filtered search and saves the second round trip.
	 * @param dsl the context to run the count with
	 * @param query the query to page over, filtered and ordered but not limited
	 * @param pageNumber the one-based page to fetch, raised to the first page when lower
