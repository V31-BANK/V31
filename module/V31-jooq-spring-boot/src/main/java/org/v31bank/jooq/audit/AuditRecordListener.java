...
/**
 * Fills in the identifier and the audit columns as records are written, so that no caller
 * has to remember to, and none can quietly decline.
 * <p>
 * This is jOOQ's counterpart to Spring Data JPA's {@code AuditingEntityListener}. It is
 * not the same mechanism: JPA works from a mapped superclass and knows every entity
 * carries the columns, whereas a jOOQ record is generated from the table it belongs to
 * and may carry none of them. Each column is therefore stamped only where the record
 * declares it, which is what lets a schema adopt auditing one table at a time.
 * <p>
 * It only sees writes made through jOOQ's record API — {@code store}, {@code
 * insert}, {@code update}, {@code merge}. A query written as {@code dsl.insertInto(...)}
 * bypasses it, because there is no record for it to act on. Where the audit trail has to
 * hold, write through records.
 * <p>
 * Timestamps come from a {@link Clock} fixed to UTC rather than from the host's zone, so
 * that rows written by nodes in different regions order against each other.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public class AuditRecordListener implements RecordListener {
	 ...
	 * @param assignIdentifiers whether to give a record without one a UUIDv7 primary key
	 */
	 ...
	 * <p>
	 * The creation columns are filled only where the record left them empty, so a merge
	 * that turns out to update an existing row does not rewrite when it was created —
	 * while a merge that inserts still records it.
	 */
	@Override
	...
	/**
	 * Record who created the row and when, and give it an identifier if it has none.
	 * <p>
	 * The identifier is left alone when the caller already chose one: a transfer whose
	 * identifier was issued upstream, so that a retry writes the same row rather than a
	 * second one, must keep it.
	 ...
	 * they describe an event that has already happened.
	 ...
	 * <p>
	 * A missing value leaves the column as it is rather than writing null over it: an
	 * unidentified caller is a reason not to claim who acted, not a reason to erase who
	 * did.
