 ...
 * <p>
 * The gRPC interceptors carry values from one service to the next, but something
 * has to put them there in the first place, and for most requests that is an HTTP
 * call from outside. Without this filter the client interceptor finds nothing to
 * send: the first service in the chain logs one identifier, the second issues
 * another, and the two halves of the same request cannot be joined up.
 * <p>
 * Runs first, so that everything after it — including anything that logs — is
 * already inside the request's context.
 * <p>
 * Named for what it does rather than for what it holds, because Spring MVC
 * already registers a {@code requestContextFilter} of its own and two beans
 * cannot share a name.
 *
 * @author Xander Wang
 * @since 0.2.0
...
public class HeaderPropagationFilter extends OncePerRequestFilter {
    ...
    /**
     * Create a filter.
     * @param propagatedHeaders the header names to carry beyond the request
     * identifier
     */
