...
/**
 * The identifier correlating everything done while serving one request, and how
 * it travels.
 * <p>
 * The same identifier the HTTP layer carries in {@link ApiHeaders#REQUEST_ID},
 * under the lower-case name gRPC requires of a metadata key — so a request that
 * arrives over REST and continues over gRPC is one trace rather than two.
 * <p>
 * It is one of the values {@link RequestContext} carries, and the only one this
 * module issues rather than merely forwarding: a request with no identifier is
 * given one at the first V31 service it reaches, because a trace that starts
 * halfway through is not much of a trace.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public final class GrpcRequestId {
    ...
    /**
     * What it travels under, in gRPC metadata and in HTTP headers alike. Lower
     * case because gRPC rejects a metadata key that is not, and HTTP header names
     * are matched case-insensitively so the same spelling works for both.
     */
     ...
     * <p>
     * It ends up in a metadata value, in a response header, and in every log line
     * of the request. An unchecked one would let a caller forge log entries by
     * embedding newlines, or push something through the metadata encoder that has
    ...
    /**
     * Return the identifier to serve a request under, keeping the caller's only
     * when it is safe to echo.
