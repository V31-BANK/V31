 ...
 * <p>
 * Every call gets a request identifier: the caller's when it sent one, a new one
 * when it did not. Anything else listed in
 * {@code v31.grpc.propagation.headers} is carried through as it arrived.
 * <p>
 * The values go into the gRPC context, which is what makes them reachable from
 * the handler and from the client interceptor when that handler calls another
 * service. The {@link MDC} is set and cleared around each callback rather than
 * once around the whole call, because the callbacks do not all run on the same
 * thread and a value left behind on a pooled thread would be attributed to
 * whatever that thread served next.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
public class HeaderPropagationServerInterceptor implements ServerInterceptor {
     ...
     * @param propagatedHeaders the names to carry through beyond the request
     * identifier, lower case as gRPC requires
     */
