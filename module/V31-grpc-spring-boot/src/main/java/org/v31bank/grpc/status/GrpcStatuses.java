 ...
 *
 * gRPC has sixteen status codes. This platform has as many error codes as it has
 * ways to refuse a request, and a caller that has to tell a duplicate code from a
 * node that still has children cannot do it from {@code ALREADY_EXISTS} alone —
 * the same problem the REST envelope solves by carrying a code beside the HTTP
 * status. So the status code is the coarse signal, understood by anything that
 * speaks gRPC, and the exact code travels in a trailer beside it.
 *
 * <h2>What goes where</h2>
 *
 * <ul>
 * <li>the status code — for load balancers, retry policies, and metrics, none of
 * which know this platform's codes</li>
 * <li>the status description — the message, which is written for whoever is
 * waiting on the call</li>
...
public final class GrpcStatuses {
    ...
    /**
     * Trailer carrying the exact {@link ErrorCode#code()} a call was refused
     * with.
     * <p>
     * Lower case because gRPC rejects a metadata key that is not, and prefixed so
     * that it cannot collide with a key some library adds.
     */
     ...
     * <p>
     * The mapping goes through the HTTP status the code already declares, so a
     * failure reported over REST and the same failure reported over gRPC agree —
     * a service does not have to decide twice what {@code NOT_FOUND} means. It
     * follows the correspondence Google publishes between HTTP statuses and the
     * canonical codes.
     * <p>
    ...
    /**
     * Return the HTTP status that says roughly what a gRPC status means, for a
     * failure that arrived without one of this platform's codes on it.
     * <p>
     * Needed when the far side is not a V31 service — a proxy that ran out of
     * capacity, a mesh that could not route the call — and there is nothing to
     * read from a trailer.
    ...
    /**
     * Return the common code standing for a gRPC status, for the same case: a
     * failure from something that is not a V31 service, which still has to reach
     * the caller as an {@link ErrorCode}.
     * @param code the status the call failed with
