...
/**
 * {@link AutoConfiguration Auto-configuration} for caching through Valkey: gives
 * Spring's cache abstraction the same JSON serialization the template uses, puts
 * every cache under this application's key prefix, and gives every cache an
 * expiry.
 *
 * <h2>Why every cache expires</h2>
 *
 * Spring's default is no expiry at all. An entry written once then stays until
 * something evicts it, and an eviction missed — a write that went round the
 * cache, a service that crashed between updating the database and clearing the
 * cache — is stale data that never heals. A balance or a limit that is quietly
 * wrong is worse than one that is briefly missing, so entries are given a life
 * and re-read when it ends. {@code v31.data.valkey.cache.ttls} sets it per cache where
 * ten minutes is the wrong answer.
 * <p>
 * This only takes effect where the application has switched caching on with
 * {@code @EnableCaching}; putting the module on the classpath does not start
 * caching anything.
 *
 * @author Xander Wang
 * @since 0.2.0
...
public class V31ValkeyCacheAutoConfiguration {
    ...
    /**
     * The configuration every cache starts from. Spring Boot picks this up and
     * builds the cache manager on it.
     * @param properties the prefix, the default expiry, and whether misses are
     * cached
    ...
    /**
     * Apply the expiry configured for individual caches, on top of the defaults
     * above.
     ...
     * <p>
     * Backs off entirely when the application supplies its own
     * {@link CachingConfigurer}, since Spring accepts only one and the
     * application's will be doing more than this.
    ...
    /**
     * Set an expiry, leaving the configuration alone when the value is not a
     * positive duration.
     * <p>
     * A zero or negative expiry is taken as a deliberate request for entries that
     * never expire, which Spring's default already does. It is not corrected here
     * — silently substituting a different lifetime for the one that was
     * configured would be worse than honouring an unwise one.
